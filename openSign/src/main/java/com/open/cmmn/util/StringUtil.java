...
/**
	...
	public static Map<String, String> getMenuCd(final String url) throws Exception {
		...
		String regExpPttrn1 = "/([a-z]+)/([a-zA-Z0-9]+)/([a-zA-Z0-9\\_]+\\.?)";
		Pattern ptn = Pattern.compile(regExpPttrn1);
		Matcher matcher = ptn.matcher(url);
		
		
		if(matcher.find()) {
			infoMap.put("menuGrp", matcher.group(1)); //1차
			infoMap.put("menuCd", matcher.group(2)); //2차
		}
		return infoMap;
	}
	...
	public static String permiSeqStr(String permiSeq) {
		String str = permiSeq;
		if(!nullConvert(str).equals("")){
			if(str.length() == 15){
				str = str.substring(0, 2) + "-" + str.substring(2, 6) + "-" + str.substring(6, 8) + "-" + str.substring(8, 15);
			}
		}
		return str;
	}
	
	public static String permiStateStr(String permiSeq) {
		String str = permiSeq;
		String reStr = "";
		if(!nullConvert(str).equals("")){
			if(str.equals("1")){
	...
	public static String permiMesDate(String permiSeq) {
		String str = permiSeq;
		if(!nullConvert(str).equals("")){
			str = str.replace("년",".").replace("월",".").replace("일","");
		}
		return str;
	}
}
