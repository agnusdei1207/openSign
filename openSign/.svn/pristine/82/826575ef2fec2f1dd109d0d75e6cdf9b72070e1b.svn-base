						...
						<tbody>
							<tr>
								<td>
									...
									<select id="schEtc01" name="schEtc01" class="text w150">
										<option value="">전체</option>
										<option value="A" <c:out value="${searchVO.schEtc01 eq 'A' ? 'selected=\"selected\"' : ''}"/>>이름</option>
										<option value="B" <c:out value="${searchVO.schEtc01 eq 'B' ? 'selected=\"selected\"' : '' }"/>>직급</option>
									</select>
									<input type="text" id="searchKeyword" name="searchKeyword" class="text w60p hideBtn3" value="<c:out value="${searchVO.searchKeyword }"/>"/>
								</td>
							</tr>
						</tbody>
					</table>
